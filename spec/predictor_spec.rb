...
describe Predictor do
  ...
  describe '#train' do
    ...
    it 'correctly approaches delta' do
      ...
      (0...10).each do
        predictor.train(delta, learning_rate + 0.5, momentum_rate + 0.5)
        predictor.build_neural_net
      end
      expect(predictor.hypothesis).to be_between(delta - 0.2, delta + 0.2)
    end
    ...
    it 'correctly predicts another stock after training' do
      delta = predictor.normalize_expected(predictor.delta)
      (0...5).each do
      ...
      predictor.set_new_features(search_term_3)
      (0...5).each do
        predictor.train(delta, learning_rate, momentum_rate)
        predictor.build_neural_net
      end

      predictor.set_features(stock_2, search_term_4, start_day, end_day)
      delta = predictor.normalize_expected(predictor.delta)
      predictor.build_neural_net
      expect(predictor.hypothesis).to be_between(delta - 0.2, delta + 0.2)
    end
